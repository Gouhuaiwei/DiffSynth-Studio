...
from diffsynth.models.wan_audio_cross_attention import AudioProjModel, FantasyTalkingAudioConditionModel
from diffsynth.models.wan_video_dit import AudioCrossAttention

os.environ["TOKENIZERS_PARALLELISM"] = "false"


def build_fantasytalking_checkpoint(fantasytalking: FantasyTalkingAudioConditionModel, wan_dit: torch.nn.Module):
    return fantasytalking.build_state_dict(wan_dit)
...
class WanFantasyTalkingTrainingModule(DiffusionTrainingModule):
    def __init__(
        ...
        if resume_from_checkpoint is not None and os.path.isfile(resume_from_checkpoint):
            self.fantasytalking.load_audio_condition_model(resume_from_checkpoint, self.pipe.dit)
            print(f"Loaded FantasyTalking checkpoint: {resume_from_checkpoint}")

        self.pipe.dit.requires_grad_(False)
        self.wav2vec.requires_grad_(False)
        self.emotion2vec.requires_grad_(False)
        self.fantasytalking.requires_grad_(False)
        self.audio_proj_model_frame.requires_grad_(True)
        self.audio_proj_model_global.requires_grad_(True)
        for module in self.pipe.dit.modules():
            if isinstance(module, AudioCrossAttention):
    ...
    def audio_proj_model_frame(self) -> AudioProjModel:
        return self.fantasytalking.proj_model_frame

    @property
    def audio_proj_model_global(self) -> AudioProjModel:
        return self.fantasytalking.proj_model_global
    ...
    def forward(self, data, inputs=None):
        if isinstance(data, list):
            losses = [self.forward(sample, inputs=None) for sample in data]
        ...
        audio_proj = self.audio_proj_model_frame(wav_feat.to(dtype=torch.bfloat16))
        audio_proj_global = self.audio_proj_model_global(emo_feat.to(dtype=torch.bfloat16))
...
def wan_parser():
    ...
    parser.add_argument("--audio_proj_dim", type=int, default=None, help="Audio projection dim. Defaults to the Wan DiT hidden dim.")
