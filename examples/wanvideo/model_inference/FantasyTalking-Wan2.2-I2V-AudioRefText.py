...
from transformers import AutoFeatureExtractor, AutoModel, Wav2Vec2Model, Wav2Vec2Processor
...
def parse_args():
    ...
    parser.add_argument("--emotion2vec_model_dir", type=str, default="audeering/wav2vec2-large-robust-12-ft-emotion-msp-dim")
...
def get_audio_features_wav2vec(wav2vec, wav2vec_processor, audio_path: str):
    ...
    inputs = {k: v.to(device=next(wav2vec.parameters()).device) for k, v in inputs.items()}
    with torch.no_grad():
        out = wav2vec(**inputs)
    return out.last_hidden_state


def get_audio_features_emotion2vec(emotion_extractor, emotion_model, audio_path: str):
    audio, sr = librosa.load(audio_path, sr=16000, mono=True)
    inputs = emotion_extractor(audio, sampling_rate=sr, return_tensors="pt", padding=True)
    inputs = {k: v.to(device=next(emotion_model.parameters()).device) for k, v in inputs.items()}
    with torch.no_grad():
        out = emotion_model(**inputs)
    return out.last_hidden_state
...
def load_models(args):
    ...
    emotion_extractor = AutoFeatureExtractor.from_pretrained(args.emotion2vec_model_dir)
    emotion_model = AutoModel.from_pretrained(args.emotion2vec_model_dir).to("cuda", dtype=torch.bfloat16)

    return pipe, fantasytalking, wav2vec_processor, wav2vec, emotion_extractor, emotion_model


def main(args, pipe, fantasytalking, wav2vec_processor, wav2vec, emotion_extractor, emotion_model):
    ...
    audio_emotion2vec_fea = get_audio_features_emotion2vec(emotion_extractor, emotion_model, args.audio_path)
...
if __name__ == "__main__":
    args = parse_args()
    pipe, fantasytalking, wav2vec_processor, wav2vec, emotion_extractor, emotion_model = load_models(args)
    out = main(args, pipe, fantasytalking, wav2vec_processor, wav2vec, emotion_extractor, emotion_model)
